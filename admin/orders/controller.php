...
function doInsert(){
			...
			if ($image_size==FALSE || $type=='video/wmv') {
				...
				redirect("index.php?view=add");
					...
					redirect('index.php?view=add');
						...
						if ($res >=1) {
							message("Car already exist!", "error");
							redirect("index.php?view=add");
						}else{
	...
	function doEdit(){
		...
		if ($_GET['actions']=='confirm') {
			...
			// $order = New Order();
				$status	= 'Cancelled';
				$remarks ='Your order has been cancelled due to certain issues. Make another order.';
		}
  	...
  	// 		$res = $customer->single_customer($_GET['customerid']); 
	        ...
	        $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$cur->PHONE."','Ronald','FROM Lira car Sellers: Your order has been '".$status. "'. The amount is '".$cur->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
			...
			foreach ($cur as $result) {
	  		 ...
	  		 $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$result->OWNERPHONE."','Ronald','FROM Lira car Sellers: Your  product has been ordered'. The amount is '".$result->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
	...
	if (isset($_POST['selector'])==''){
		...
		for($i=0;$i<$key;$i++){
			...
			message("Car has been Deleted!","info");
			redirect('index.php?view=add');
		}
   ...
   if(isset($_GET['id'])){
    ...
    $price= $_GET['price'];
      ...
      addtocart($pid,1,$price);
			...
			message("1 car has been added in the cart", "success");
			redirect("index.php?view=add");
	...
	function cartDelete(){
		...
		message("1 car has been removed in the cart.");
